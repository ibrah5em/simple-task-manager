...
<div class="d-flex align-items-center justify-content-between mb-4 anim-fade-up flex-wrap gap-3">
    <div>
        <h4 class="fw-bold mb-1 d-flex align-items-center gap-2" style="color:var(--text)">
            <i class="bi bi-calendar-week" style="color:var(--purple-500);"></i> Upcoming
        </h4>
...
@if($tasks->isEmpty())
    <div class="glass-card p-5 text-center anim-fade-up anim-delay-1">
        <i class="bi bi-calendar-x" style="font-size:3rem;color:var(--purple-400);opacity:.6;"></i>
        <p class="mt-3 mb-2 fw-semibold" style="color:var(--text)">Nothing due this week</p>
        <p style="color:var(--text-muted);font-size:.875rem">No tasks are scheduled for the next 7 days.</p>
        <a href="{{ route('tasks.create') }}" class="btn-purple btn mt-1">
            <i class="bi bi-plus-circle me-1"></i> Create Task
        </a>
    </div>
@else
    @foreach($grouped as $date => $dateTasks)
    <div class="mb-4 anim-fade-up" style="animation-delay: {{ $loop->index * 0.08 }}s;">
        <div class="mb-2" style="color:var(--text-muted);font-size:.8rem;font-weight:600;text-transform:uppercase;letter-spacing:.06em;">
        ...
        <div class="glass-card p-2">
            <div class="table-wrap">
                <table class="task-table">
                    ...
                    <tbody>
                        @foreach($dateTasks as $i => $task)
                        @php
                            $priorityColors = ['high'=>'#ef4444','medium'=>'#f59e0b','low'=>'#10b981'];
                            $isOverdue = !$task->is_completed && $task->due_date && $task->due_date->lt(today());
